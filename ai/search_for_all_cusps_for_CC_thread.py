...
class SearchForAllCuspsForCCThread(threading.Thread):
    ...
    def run(self):
        ...
        if len(self.app.editor_fen_text.get("1.0", END)) > 1:
            self.app.editor_fen_text.insert(END, "\n" + "fight starting positions for Cusp Chess")
        else:    
            self.app.editor_fen_text.insert(END, "fight starting positions for Cusp Chess")
        try:    
        ...
        messagebox.showinfo("Search Finished", "All fight starting positions search finished" )
