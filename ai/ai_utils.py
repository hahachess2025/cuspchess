...
One_cusp is for Cusp Chess game. 
...
def search_for_cusp_positions_for_CC( cusp_app, board_fen, AI_engine, search_mode, stop_flag ):
    ...
    if early_check_enable ==1:
        if castling_check_CC(cusp_app,new_board, AI_engine, search_mode, stop_flag):
            return True   
    ...
    if cusp_app.game_in_progress or search_mode == "all_cusps":
        ...
        for start_piece_position in position_list:
            ...
            if new_board.piece_at(start_piece_position):
                ...
                if early_check_enable==1:
                    # Remove the piece and change board turn to check whether it is a good fight starting position
                    if remove_a_piece_and_check_CC(cusp_app,new_board,start_piece_position, AI_engine, search_mode, stop_flag):
                        return True

                early_check_enable=1 
                
                #setup-rule to find a fight starting position. 
                piece=new_board.piece_at(start_piece_position) 

                if not cusp_app.engine_test_mode_enable: 
                    # Human-Level-Mode, a player can only move a pawn of their own 
                    # onto an empty square on the opponent's half board 
...
def castling_check_CC(cusp_app,new_board, AI_engine, search_mode, stop_flag):
    logger.info("castling_check_CC")
    output_board = new_board.copy()
    if output_board.turn:
        move_castling = chess.Move.from_uci("e1g1")
        if move_castling in output_board.legal_moves:
            output_board.push(move_castling)
            if check_fen_cusp( cusp_app, output_board.fen(), AI_engine, search_mode, stop_flag):
                convert_start_end_postion_uci(cusp_app, 4, 6)
                if search_mode == "one_cusp":
                    return True
        ...
        move_castling = chess.Move.from_uci("e1c1")
        if move_castling in output_board.legal_moves:
            output_board.push(move_castling)
            if check_fen_cusp( cusp_app, output_board.fen(), AI_engine, search_mode, stop_flag):
                convert_start_end_postion_uci(cusp_app, 4, 2)
                if search_mode == "one_cusp":
                    return True
                elif search_mode == "all_cusps":
                    output_notation_for_cusp_position( cusp_app)
        ...
        move_castling = chess.Move.from_uci("e8g8")
        if move_castling in output_board.legal_moves:
            output_board.push(move_castling)
            if check_fen_cusp( cusp_app, output_board.fen(), AI_engine, search_mode, stop_flag):
                convert_start_end_postion_uci(cusp_app, 60, 62)
                if search_mode == "one_cusp":
                    return True
                elif search_mode == "all_cusps":
                    output_notation_for_cusp_position( cusp_app)
            output_board = new_board.copy()
        move_castling = chess.Move.from_uci("e8c8")
        if move_castling in output_board.legal_moves:
            output_board.push(move_castling)
            if check_fen_cusp( cusp_app, output_board.fen(), AI_engine, search_mode, stop_flag):
                convert_start_end_postion_uci(cusp_app, 60, 58)
                if search_mode == "one_cusp":
                    return True
                elif search_mode == "all_cusps":
                    output_notation_for_cusp_position( cusp_app)
                    
    # check_CC, to check whether it is a good fight starting position.           
...
def one_free_move_CC(cusp_app,new_board, start_piece_position, AI_engine, search_mode, stop_flag):
    ...
    white_pawn_promotion_list = ["Q", "R", "N", "B"]
    black_pawn_promotion_list = ["q", "r", "n", "b"]
    
    while end_position_one_loop_finished == False:
        # Brute force
        # human-level mode, only empty positions and only the opponent's half board.
        if not cusp_app.engine_test_mode_enable:
            output_board = new_board.copy()
            pieces_position_list = list(output_board.piece_map().keys())
            end_available_position_list = [x for x in range(0, 64) if x not in pieces_position_list]
        else:
            # engine-test-mode, any position except illegal positions in standard chess.
            end_available_position_list= list(range(64))
        random.shuffle(end_available_position_list)    
        for piece_end_position in end_available_position_list:
            if search_mode == "one_cusp":
                if not cusp_app.game_in_progress: return
            if stop_flag.is_set(): return
            
            output_board = new_board.copy()

            end_position_try_number = end_position_try_number + 1
            if end_position_try_number > 64:
                end_position_one_loop_finished = True
                break
            piece= str(output_board.piece_at(start_piece_position))
            if check_pawn_illegal_move(piece,piece_end_position):continue
            if check_end_position_king(output_board,piece_end_position):continue

            # In engine-test-mode, a bishop can't be relocated onto an opposite-color square.
            if piece =='B' or piece=='b':
                if are_different_colors(start_piece_position, piece_end_position):
                    continue
            
            # human-level-mode, only empty positions and only in the opponent's half board.
            if not cusp_app.engine_test_mode_enable:
                if (new_board.turn and piece_end_position<32) or (not new_board.turn and piece_end_position>31): continue
            # for pawn promotion
            if ( piece == "p" and piece_end_position < 8 ) or ( piece == "P" and piece_end_position > 55 ):
                if not cusp_app.engine_test_mode_enable: continue
                if ( piece == "p" ):
                    for num in range(4):
                        output_board = new_board.copy()
                        output_board.remove_piece_at( start_piece_position )
                        output_board.set_piece_at( piece_end_position, chess.Piece.from_symbol( black_pawn_promotion_list[num]), )
                        if check_cusp_and_setup_with_pawn_promotion_CC( cusp_app, output_board.fen(), start_piece_position, piece_end_position,str( black_pawn_promotion_list[num] ), AI_engine, search_mode, stop_flag ):
                            return True
                        output_board.turn = 1 ^ output_board.turn
                        if check_cusp_and_setup_with_pawn_promotion_CC( cusp_app, output_board.fen(), start_piece_position, piece_end_position,str( black_pawn_promotion_list[num] ), AI_engine, search_mode, stop_flag ):
                            return True
                    ...
                    for num in range(4):
                        output_board = new_board.copy()
                        output_board.remove_piece_at( start_piece_position )
                        output_board.set_piece_at( piece_end_position, chess.Piece.from_symbol( white_pawn_promotion_list[num]), )
                        if check_cusp_and_setup_with_pawn_promotion_CC( cusp_app, output_board.fen(), start_piece_position, piece_end_position,str( white_pawn_promotion_list[num] ), AI_engine, search_mode, stop_flag ):
                            return True
                        output_board.turn = 1 ^ output_board.turn
                        if check_cusp_and_setup_with_pawn_promotion_CC( cusp_app, output_board.fen(), start_piece_position, piece_end_position,str(white_pawn_promotion_list), AI_engine, search_mode, stop_flag ):
                            return True
            else:
                ...
                output_board.turn = 1 ^ output_board.turn
                if check_cusp_and_setup_no_promotion_CC( cusp_app, output_board.fen(), start_piece_position, piece_end_position,AI_engine, search_mode, stop_flag ):
                    return True
                if not cusp_app.engine_test_mode_enable: continue
                output_board.turn = 1 ^ output_board.turn
                if check_cusp_and_setup_no_promotion_CC( cusp_app, output_board.fen(), start_piece_position, piece_end_position,AI_engine, search_mode, stop_flag ):
...
def are_different_colors(sq1, sq2) -> bool:
    # A square's index parity matches its color alignment on the board
    return (sq1 + chess.square_rank(sq1) + sq2 + chess.square_rank(sq2)) % 2 != 0

                        
def check_cusp_and_setup_with_pawn_promotion_CC( cusp_app, output_board_fen, start_piece_position, piece_end_position,pawn_promotion_piece, AI_engine, search_mode, stop_flag ):
    logger.info("check_cusp_and_setup_with_pawn_promotion_CC")
    if check_fen_cusp( cusp_app, output_board_fen, AI_engine, search_mode, stop_flag ):
        convert_start_end_postion_uci( cusp_app, start_piece_position, piece_end_position, )
        cusp_app.move_str = ( cusp_app.move_str + pawn_promotion_piece  )
        if search_mode == "one_cusp":
            return True
        elif search_mode == "all_cusps":
            output_notation_for_cusp_position( cusp_app)    

def check_cusp_and_setup_no_promotion_CC( cusp_app, output_board_fen, start_piece_position, piece_end_position,AI_engine, search_mode, stop_flag ):
    logger.info("check_cusp_and_setup_no_promotion_CC")
    if check_fen_cusp( cusp_app, output_board_fen, AI_engine, search_mode, stop_flag ):
        convert_start_end_postion_uci( cusp_app, start_piece_position, piece_end_position, )
        if search_mode == "one_cusp":
            return True
...
We can find a position's score within an acceptable range, when the Score Difference is small.
The Score Difference is: abs(abs(score)-1).
We can set Score Difference range: Maximum Score Difference and Minimum Score Difference
...
Position B's score is -2, white to move. Stockfish 1.0 choose Black, and white must win. 
...
def check_fen_cusp(cusp_app, old_fen, AI_engine, search_mode, stop_flag):
    ...
    board = chess.Board(engine_fen)
    board_turn_changed = chess.Board(engine_fen)
    ...
    if board.is_checkmate() == False and board_turn_changed.is_check() == False:
        ...
        try:
            ...
            if stop_flag.is_set(): return
            cusp_app.after(0, lambda err=e:  messagebox.showerror("Engine error", f"When checking whether a FEN is a good fight starting position: {err}"))
            return
        if stop_flag.is_set(): return
        if ( ( cusp_app.engine_score_cusp_inner_range_enable and 1 - cusp_app.engine_score_difference_maximum - first_threshold < position_score <= 1 - cusp_app.engine_score_difference_minimum + first_threshold ) 
            ...
            try:
                ...
                if stop_flag.is_set(): return
                cusp_app.after(0, lambda err=e: messagebox.showerror("Engine Error", f"When checking whether a FEN is a fight starting position: {err}"))
                return
...
# When we find a fight starting position, we need to set color-to-move(if in engine-test-mode) and color-must-win.                    
def set_up_for_the_cusp_position(cusp_app,position_score,board,search_mode):
    ...
    if search_mode == "one_cusp":
        ...
        if cusp_app.engine == cusp_app.engine_one:
            # we need to know which color set up the fight starting position, so we can let the other to choose a color.
            cusp_app.active_color_in_cusp_setup = "W"
...
def convert_start_end_postion_uci( cusp_app, start_piece_position, piece_end_position):
    ...
    if piece_end_position != -1:
        cusp_app.move_str = str(chess.square_name(start_piece_position)) + str( chess.square_name(piece_end_position) )
    else:
        cusp_app.move_str = str(chess.square_name(start_piece_position)) + "xx"
...
# For searching for all good fight starting positions
def output_notation_for_cusp_position(cusp_app):
    ...
    if cusp_app.color_to_move_in_fight_phase == "W":
        ...
        color_to_move = "B"
    # a complete example "CC {WS e2e4 WWBN}", white setup, e2e4, white must win, black to move
...
def check_pawn_illegal_move(piece,piece_end_position):
    logger.info("check_pawn_illegal_move")
    piece=str(piece)
    if ( piece == "p" or piece == "P" ) and ( chess.square_rank(piece_end_position) == 0 or chess.square_rank(piece_end_position) == 7 ):
        return True
