...
For stockfish engine and many other chess engines, +1 means win rate is aournd 50%, and -1 means loss rate is 50%.
...
class SafeMoveOrSetupThread(threading.Thread):
    ...
    def run(self):
        ...
        if self.app.chess_game_variant_mode != "Normal" :
            while both_AI_players and self.app.game_in_progress:
                ...
                cusp_chess_player_one_turn = ( self.app.chess_game_variant_mode == "CuspChess" and self.app.board.turn)
                cusp_chess_player_two_turn = ( self.app.chess_game_variant_mode == "CuspChess" and not self.app.board.turn )

                if ( self.app.cusp_chess_phase == "SafeMove" and ( ( self.app.player_one == "AI" and cusp_chess_player_one_turn  ) 
                    or ( self.app.player_two == "AI" and  cusp_chess_player_two_turn ) ) ) or self.app.cusp_chess_phase == "Decision":
                    ...
                    # in Decision Phase, we need to know the color who set up a fight starting position.
                    ...
                    if self.app.cusp_chess_phase == "SafeMove":
                        # Based on chess engine Stockfish, score 1 means win rate is around 50% for the color to move, and -1 means loss rate is around 50%
                        # A engine can choose the color directly if score > 1 or score < -1.
                        # This can only happen when a player made a mistake before to let the score >1 or <-1, because the starting position is a draw.
                        ...
                        # choose your opponent's color
                            ...
                            # AI will search for a fight starting position 
                            # when on its turn and board ply is not smaller than maximum ply.
                            self.choose_safe_move_or_setup()

                    elif self.app.cusp_chess_phase == "Decision":
                        # The score is the only reason which color a player will choose 
                        # after its opponent set up a fight starting position.                       
                        # It is drawed as green mark on the player's eval bar.
                        self.passive_AI_player_set_cusp_score(position_score)
                        
                        # A player chooses a color, when his/her opponent set up a fight starting position
    ...
    So when a player chooses a color, the color-must-win and color-to-move are set. 
    ...
    def choose_the_current_color_directly(self,position_score ):
        ...
        if self.app.engine == self.app.engine_one:
            # The score worths remembered. We draw a red mark
            # for active color and green mark for passive color on eval bars.
        ...
        # Cusp Chess    
        if self.app.chess_game_variant_mode == "CuspChess":
            # Now score > 1, it means white can win. That is why we choose white now.
            # After we chose white directly, white must win.
        ...
        # set the CC prefix in move notation
    ...
    # A tentative white player doesn't need to choose white. He/She only need to choose a color to win.
    def choose_the_opposite_color_directly(self,position_score ):
        ...
        if self.app.game_in_progress:
            ...
            ui.ui_utils.draw_pieces( self.app, self.app.chess_game_variant_mode)
            # it is time for the opponent to move,
            self.app.update()
    ...
    based on his/her chess skills and his/her opponents. 
    Here we randomly choose to make a Safe Move or set up a fight starting position. Engines don't know the difference.
    To get into Fight phase faster, we set some constraints in the program, which is not necessary in a real Cusp Chess game.
    """
    def choose_safe_move_or_setup(self):
        logger.info("choose_safe_move_or_setup")
        if ( ( self.app.chess_game_variant_mode == "CuspChess" and self.app.board.ply() < self.app.maximum_ply_before_setup )  
    ...
    If the color's score > 1, the color can win, and the AI will chooses the color. 
    ...
    def passive_AI_player_choose_color(self, position_score):
        ...
        if not self.app.choose_the_recommended_color_enable:
            if self.app.color_must_win_in_cusp_chess == "W":
                ...
                # The board's FEN is set as a fight starting position. It only means white to move in Fight phase.
                if self.app.board.turn:
                    # white will win
                    if position_score >= 1:
                ...
                if not self.app.board.turn:
                    ...
                    # black will lose
                        ...
                        if self.app.engine == self.app.engine_two:
                            ...
                            self.app.player_swap_side = False
        # useful for two engines to play handicap games. for some engines, one doesn't mean win rate is 50%.
        else:
    ...
    def game_get_into_fight_phase(self): 
        ...
        if self.app.game_in_progress:
            ...
            if not self.app.player_swap_side:
                ...
                if ( self.app.color_to_move_in_fight_phase == "W" and self.app.player_two == "AI" ) or ( self.app.color_to_move_in_fight_phase == "B" and self.app.player_one == "AI" ):
                    self.app.AI_searching_best_move()
    # If not setting up a fight starting position, we can make a Safe Move: -1<score< 1.
            ...
            for move in list_legal_moves:
                ...
                if self.app.game_in_progress:
                    ...
                    try:
                        ...
                        return
                    # to show 'choosing a color directly', we can set a bigger range    
                    ...
                    if self.app.engine_safe_move_score_maximum>position_score > -self.app.engine_safe_move_score_maximum:    
                        ...
                        self.app.move_str = str( self.app.board.san(move))
                        self.app.move_score = -position_score
                        self.app.move_score_set = True
                        utils.pgnhistory.save_PGN_and_output_move_history( self.app)
    ...
    # Search for a fight starting position and set color-to-move and color-must-win.
    def setup_cusp_postion_for_CC(self):
        ...
        # find a good fight starting position in real time based on current board position. 
    ...
    def check_if_pawn_positions_are_legal(self, board):
        logger.info("check_if_pawn_positions_are_legal") 
        pawn_positions = [
            k for k, v in board.piece_map().items() if ( str(v) == "p" or str(v) == "P")]
        if len(pawn_positions) > 0:
            for pawn_position in pawn_positions:
                if ( chess.square_rank(pawn_position) == 0 or chess.square_rank(pawn_position) == 7 ):
                    return False
        return True
