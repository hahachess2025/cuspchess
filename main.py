"""
Cusp Chess: The ultimate Drawless Chess
...
Cusp Chess is a decisive chess variant based on Freestyle Player-Balancing (FSPB). 
...
2. Components
2.1	A standard 8×8 chessboard.
2.2	Standard chess pieces for each player.

====================================================================

3. Initial setup
3.1	Standard chess starting position.
3.2	Determine who are tentative white player and tentative black player by mutual agreement or random choice.
3.3	Piece Movement: All pieces (King, Queen, Rook, Bishop, Knight, Pawn) move and capture exactly as they do in standard FIDE chess, except in Decision Phase.
...
4.3	The default must-win color is White. If no one set up a fight-starting position when the game is over, draw means loss for the tentative white player.
...
•	Safe position: if win rates of both color of a position are below 50%, the position is a safe position. 
    ...
    Based on popular chess engines, the score of a safe position ranges from -1 to +1.
...
•	Fight-starting position: It is the starting position in the Fight Phase. 
    ...
    if no one did it, draw means loss for the tentative white player.
...
7. Moves
•	Legal move: In Cusp Chess, there are three types of legal moves for three phases respectively: 
    legal safe moves, legal setup moves, and legal fight moves. 
    The legal safe move and legal fight move are the same as legal moves in standard chess. 
...
•	One-Free move: All moves that are illegal in standard chess 
...
All standard chess rules apply, except draw means loss for the must-win color.
...
9.2 One-Free move
One-Free move is illegal in standard chess. It is designed to create more good fight-starting positions in Cusp Chess by relocating at most one piece. There are two modes for One-Free move in Cusp Chess: Human-Level-Mode and Engine-Test-Mode.
...
•	If the must-win color is not set up in a game, White is the must-win color by default. Draw means loss for the tentative white player, because it is the tentative white player’s responsibility to set up a fight-starting position.

====================================================================             
10. Notation system

Three notation examples 
            
A, Moving a piece to set up a fight starting Position.

1. CC {WS e2f5 BWBN} {0.97} 
2. CC {BCBW} {1.07} 
3. ... d6 {1.07} 
4. d4 {-1.07}  Bxf5 {1.08} 
5. g4 {-1.0}  Bg6 {1.1}  
...
B, Removing a piece to set up a fight starting Position.

1. a4 {-0.23}  a5 {-0.4} 
2. CC {WS d7xx WWBN} {-1.07} 
3. CC {BCBD} {-0.91} 
4. ... e5 {-0.99} 
5. e3 {0.95}  Bc5 {-1.06} 
6. b3 {1.15}  Nf6 {-1.06} 

C, Choosing a color directly to set up a fight starting Position.

1. g3 {0.12}  f5 {-0.42} 
2. f3 {-0.61}  Na6 {-0.49} 
3. a3 {-0.2}  Nf6 {-0.21} 
4. Ra2 {-1.09} 
5. CC {BC none BWBN} {1.04} 
6. ... e5 {1.01} 
7. e3 {-1.05}  d5 {0.96} 
8. d4 {-1.23}  e4 {1.06} 
...
====================================================================
For more details, please check the paper about FSPB and Cusp Chess official rulebook.           
====================================================================
"""
...
class CuspChessApp(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("Cusp Chess")
        self.geometry("1550x900")
        self.resizable(True, True)
        self.resizing_enabled = False
        
        self.icon_img = PhotoImage(file="assets/CuspChessLogo.png")
    ...
    def create_Boards_menu(self):
        ...
        self.Boards_menu.add_command( label="Chess", command=self.go_back_to_chess)
        self.Boards_menu.add_command( label="Board editor", command=self.go_to_editor_board )
        self.Boards_menu.add_command( label="Blindfold Chess", command=self.go_to_blindfold_chess )
    ...
    def create_language_menu(self):
        ...
        self.language_menu.add_command( label="Deutsch", command=self.set_to_German )
        self.language_menu.add_command( label="Français", command=self.set_to_French )
        self.language_menu.add_command( label="Español", command=self.set_to_Spanish )
        self.language_menu.add_command( label="Українська", command=self.set_to_Ukrainian )        
        self.language_menu.add_command( label="Русский", command=self.set_to_Russian )
    ...
    def set_to_German(self):
        logger.info("set_to_German")
    ...
    def set_to_Spanish(self):
        logger.info("set_to_Spanish")
        self.current_lang = "es"
        utils.config.save_setting_in_config_file(self)
        ui.language.update_texts(self)
        logger.info(f"self.current_lang: {self.current_lang}")

    def set_to_Ukrainian(self):
        logger.info("set_to_Ukrainian")
        self.current_lang = "ukr"
        utils.config.save_setting_in_config_file(self)
        ui.language.update_texts(self)
        logger.info(f"self.current_lang: {self.current_lang}")
        
    def set_to_Russian(self):
        logger.info("set_to_Russian")
    ...
    def go_to_blindfold_chess(self):
        ...
        if not self.blindfold_chess_frame:
            ui.blindfold.create_blindfold_chess_frame(self)
        else:
            self.blindfold_chess_frame.tkraise()
    ...
    def start_game(self):
        ...
        if self.chess_game_variant_mode == "CuspChess":
