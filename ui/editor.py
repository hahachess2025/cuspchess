...
def create_editor_board_frame(cusp_app):
    ...
    cusp_app.editor_board_canvas = Canvas( cusp_app.editor_board_frame, width=int(cusp_app.editor_canvas_size), height=int(cusp_app.editor_canvas_size * 10 / 8), )
    ...
    cusp_app.editor_player_one_bar = Canvas(
        ...
        width=20,
        height=cusp_app.editor_canvas_size,
        bg="#808080",
    )
...
def create_editor_setting_UI(cusp_app):
    ...
    cusp_app.editor_engine_search_for_cusps_label = ttk.Label( cusp_app.editor_setting_UI, justify=LEFT, compound=LEFT, font=("Times", 12), )
    ui.language.register_widget(cusp_app, cusp_app.editor_engine_search_for_cusps_label, "editor_engine_search_for_cusps_label")
    cusp_app.editor_engine_search_for_cusps_label.grid( column=0, row=11, padx=5, pady=10)

    cusp_app.editor_search_for_cusps_for_CC_confirm_button = tk.Button( cusp_app.editor_setting_UI, command=lambda: editor_engine_search_for_cusps_confirm( cusp_app, "CuspChess"), width=30, )
...
def editor_engine_search_for_cusps(cusp_app, chess_variant):
    ...
    if cusp_app.editor_engine_analyse_enable:
        ...
        try:
            if chess_variant == "CuspChess":
                cusp_app.search_for_all_cusps_for_CC_thread = ( ai.search_for_all_cusps_for_CC_thread.SearchForAllCuspsForCCThread( app=cusp_app))
                cusp_app.search_for_all_cusps_for_CC_thread.start()
        except Exception as e:
            logger.exception("Error: editor_engine_search_for_cusps for Cusp Chess ")
            messagebox.showerror("Error", f"Error when searching all fight starting positions for Cusp Chess: {e}" )
...
def editor_draw_chessboard(cusp_app):
    ...
    editor_board_img = PILImage.open("assets/10rankchessBoardnotationall.png")
    editor_board_img = editor_board_img.resize( (int( cusp_app.editor_canvas_size), int( cusp_app.editor_canvas_size * 10 / 8)), PILImage.Resampling.LANCZOS, )
...
def editor_resize_board(cusp_app, event):
    ...
    cusp_app.editor_canvas_size = appheight * (78 / 100)  # 73
    cusp_app.editor_canvas_size = int(cusp_app.editor_canvas_size / 8) * 8

    cusp_app.editor_board_canvas.config( width=cusp_app.editor_canvas_size, height=int(cusp_app.editor_canvas_size * 10 / 8), )
    cusp_app.editor_player_one_bar.config( width=20, height=cusp_app.editor_canvas_size)
...
def editor_board_left_click(cusp_app, event):
    ...
    if ( mouse_x >= cusp_app.editor_canvas_size or mouse_x <= 0 or mouse_y >= cusp_app.editor_canvas_size * 10 / 8 or mouse_y <= 0 ):
        return

    canvas_x = mouse_x // (cusp_app.editor_canvas_size / 8)
    canvas_y = mouse_y // (cusp_app.editor_canvas_size / 8)
    canvas_x = int(canvas_x)
    canvas_y = int(canvas_y)

    if (cusp_app.editor_canvas_size / 8 < mouse_y < cusp_app.editor_canvas_size * 9 / 8) or ((0 < mouse_y < cusp_app.editor_canvas_size / 8 or cusp_app.editor_canvas_size * 9 / 8 < mouse_y < cusp_app.editor_canvas_size * 10 / 8) and cusp_app.editor_canvas_size * 7 / 8 > mouse_x > cusp_app.editor_canvas_size / 8):
        chessboard_x = canvas_x
        chessboard_y = 9 - canvas_y
        ten_rank_chessboard_index = chessboard_x + chessboard_y * 8
        if ( cusp_app.editor_canvas_size / 8 < mouse_y < cusp_app.editor_canvas_size * 9 / 8 ):
            piece = cusp_app.board.piece_at(ten_rank_chessboard_index - 8)
            if not piece:
                return

        cusp_app.piece_move_start_square = ten_rank_chessboard_index
...
def editor_board_left_button_release(cusp_app, event):
    ...
    mouse_x, mouse_y = event.x, event.y
    # a move on chess board
    if (cusp_app.editor_canvas_size / 8 < mouse_y < cusp_app.editor_canvas_size * 9 / 8 and cusp_app.editor_canvas_size > mouse_x > 0):
        canvas_x = mouse_x // (cusp_app.editor_canvas_size / 8)
        canvas_y = mouse_y // (cusp_app.editor_canvas_size / 8)
        canvas_x = int(canvas_x)
        canvas_y = int(canvas_y)
        ...
        if 0 < cusp_app.piece_move_start_square < 7:
            # add white pieces
            if cusp_app.piece_move_start_square == 1:
                cusp_app.board.set_piece_at( ten_rank_chessboard_index - 8, chess.Piece.from_symbol("R") )
            elif cusp_app.piece_move_start_square == 2:
                cusp_app.board.set_piece_at( ten_rank_chessboard_index - 8, chess.Piece.from_symbol("N") )
            elif cusp_app.piece_move_start_square == 3:
                cusp_app.board.set_piece_at( ten_rank_chessboard_index - 8, chess.Piece.from_symbol("B") )
            elif cusp_app.piece_move_start_square == 4:
                cusp_app.board.set_piece_at( ten_rank_chessboard_index - 8, chess.Piece.from_symbol("Q") )
            elif cusp_app.piece_move_start_square == 5:
                cusp_app.board.set_piece_at( ten_rank_chessboard_index - 8, chess.Piece.from_symbol("P") )
            elif cusp_app.piece_move_start_square == 6:
                cusp_app.board.set_piece_at( ten_rank_chessboard_index - 8, chess.Piece.from_symbol("K") )
        elif 72 < cusp_app.piece_move_start_square < 79:
            # add Black pieces
            if cusp_app.piece_move_start_square == 73:
                cusp_app.board.set_piece_at( ten_rank_chessboard_index - 8, chess.Piece.from_symbol("r") )
            elif cusp_app.piece_move_start_square == 74:
                cusp_app.board.set_piece_at( ten_rank_chessboard_index - 8, chess.Piece.from_symbol("n") )
            elif cusp_app.piece_move_start_square == 75:
                cusp_app.board.set_piece_at( ten_rank_chessboard_index - 8, chess.Piece.from_symbol("b") )
            elif cusp_app.piece_move_start_square == 76:
                cusp_app.board.set_piece_at( ten_rank_chessboard_index - 8, chess.Piece.from_symbol("q") )
            elif cusp_app.piece_move_start_square == 77:
                cusp_app.board.set_piece_at( ten_rank_chessboard_index - 8, chess.Piece.from_symbol("p") )
            elif cusp_app.piece_move_start_square == 78:
                cusp_app.board.set_piece_at( ten_rank_chessboard_index - 8, chess.Piece.from_symbol("k") )

        elif 7 < cusp_app.piece_move_start_square < 72:
            # move pieces
            piece = cusp_app.board.piece_at( cusp_app.piece_move_start_square - 8)
            if piece and cusp_app.piece_move_start_square!=ten_rank_chessboard_index:
                if cusp_app.editor_auto_turn_rotation:
                    ...
                    if cusp_app.board.turn:
                        ...
                        cusp_app.editor_radio_value.set(0)                      
                cusp_app.board.remove_piece_at( cusp_app.piece_move_start_square - 8)
                cusp_app.board.set_piece_at( ten_rank_chessboard_index - 8, piece)
...
def editor_update_player_score_bar(cusp_app):
    ...
    if cusp_app.editor_engine_analyse_enable:
        try:        
            if cusp_app.editor_engine:
                cusp_app.editor_engine.reset()
            cusp_app.update_editor_score_thread = ai.update_editor_score_thread.UpdateEditorScoreThread( app=cusp_app)
